...
def load_data():
    ...
    df_agg['Average view duration'] = df_agg['Average view duration'].apply(
        lambda x: datetime.strptime(x, '%H:%M:%S') if isinstance(x, str) else x
    )
    ...
    df_agg['Views / sub gained'] = df_agg['Views'] / df_agg['Subscribers gained'].replace(0, np.nan)
...
# Daily views: mean, median, percentiles
...
if add_sidebar == 'Aggregate Metrics':
    df_agg_metrics = df_agg[['Video publish time','Views','Likes','Subscribers','Shares',
    ...
    # Show metrics
    cols = st.columns(5)
    for idx, i in enumerate(metric_medians6mo.index):
...
if add_sidebar == 'Individual Video Analysis':
    ...
    # Bar chart (audience by country + subscription)
    ...
    fig2.add_trace(go.Scatter(x=first_30['days_published'], y=first_30['Views'].cumsum(),
                              mode='lines', name='Current Video', line=dict(color='firebrick', width=4)))
