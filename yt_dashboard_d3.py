...
# @st.cache
# st.cache_data:  for caching pure data objects (like Pandas DataFrames, NumPy arrays, etc.)
# st.cache_resource :for caching resources that should be reused across reruns (like database connections, ML models, etc.).
...
def load_data():
    ...
    df_agg['Video publish time'] = pd.to_datetime(df_agg['Video publish time'], format='mixed', errors='coerce')
...
median_agg = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo].median(numeric_only=True) # > 2022
...
#Engineer data for time series.................................... B.
...
# fig = px.bar(df_agg, x="Video title", y="Views", title="Views per Video")
# st.plotly_chart(fig)
